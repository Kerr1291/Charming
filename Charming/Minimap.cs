...
namespace CharmingMod
{
    public class Minimap
    {
        ...
        public Minimap(GameMap GameMap)
        {
            ...
            CharmingMod.Instance.Log("Setting up the minimap");
        ...
        private void SetupCamera(bool create = true)
        {
        ...
        {
            CharmingMod.Instance.Log("Setting up areas");
            if (areas != null)
            {
                ...
                if (pins.Keys.Count > 0)
                {
                    foreach (string key in pins.Keys)
                    {
                        GameObject.Destroy(pins[key]);
                    }
                }
                if (markers.Keys.Count > 0)
                {
                    ...
                    {
                        foreach (GameObject o in markers[key])
                        {
            ...
            CharmingMod.Instance.Log("Setting up the areas to child of the minimap");
            foreach (string area in areas.Keys)
            {
        ...
        public void MapRoom(string name)
        {
            CharmingMod.Instance.Log("MapRoom: " + name);
            PlayerData pd = PlayerData.instance;
            GameObject replacement = null;

            for (int i = 0; i < map.transform.childCount; i++)
            {
                GameObject gameObject = map.transform.GetChild(i).gameObject;
                for (int j = 0; j < gameObject.transform.childCount; j++)
                {
                    GameObject gameObject2 = gameObject.transform.GetChild(j).gameObject;
                    if (pd.hasQuill && gameObject2.name == name)
                    {
                        CharmingMod.Instance.Log("Original room matched with name: " + gameObject2.name);
                        gameObject2.SetActive(true);
                        replacement = gameObject2;
                    }
                    for (int k = 0; k < gameObject2.transform.childCount; k++)
                    {
                        UnityEngine.GameObject gameObject3 = gameObject2.transform.GetChild(k).gameObject;
            ...
            for (int i = 0; i < minimapParent.transform.childCount; i++)
            {
                ...
                for (int j = 0; j < gameObject.transform.childCount; j++)
                {
                    ...
                    if (pd.hasQuill && gameObject2.name == name)
                    {
                        CharmingMod.Instance.Log("Minimap room matched with name: " + gameObject2.name);
                        if (replacement != null)
                        {
                            Transform parent = gameObject2.transform.parent;
                            Vector3 localpos = gameObject2.transform.localPosition;
                            GameObject.Destroy(gameObject2);

                            gameObject2 = GameObject.Instantiate(replacement);
                            gameObject2.transform.SetParent(parent);
                            gameObject2.transform.localPosition = localpos;
                        }
        ...
        public void UpdateAreas()
        {
            ...
            if (!map.inRoom && currentMapZone != "DREAM_WORLD" && currentMapZone != "WHITE_PALACE" && currentMapZone != "GODS_GLORY")
            {
                if (PlayerData.instance.hasQuill)
                {
                    ...
                    MapRoom(GameManager.instance.sceneName);
                }
            }
            map.displayNextArea = false;
            
            List<string> keys = new List<string>(areas.Keys);
        ...
        public void UpdateMap()
        {
            ...
            string targetCompassAnim = HeroController.instance.move_input == 0 ? "Idle" : "Walk";
